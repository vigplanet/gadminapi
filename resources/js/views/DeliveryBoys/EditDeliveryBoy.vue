                                    ...
                                    <div v-if="deliveryBoys.bonus_type == 1" class="col-md-3">
                                        <div class="form-group">
                                            ...
                                            <input type="number" min="0" step="0.1" required class="form-control" name="bonus_min_amount"
                                                   ...
                                                   placeholder='Minimum bonus amount' @input="validateBonusMinAmount"/>
                                            <span class="text text-primary font-size-13">{{ __('set_0_if_you_want_to_remove_limit') }}</span>
                                            <span v-if="bonusMinAmountValidationError" class="error">{{ bonusMinAmountValidationError }}</span>
                                        </div>
                                    </div>
                                    <div v-if="deliveryBoys.bonus_type == 1" class="col-md-3">
                                        <div class="form-group">
                                            ...
                                            <input type="number" min="0" step="0.1" required class="form-control" name="bonus_max_amount"
                                                   ...
                                                   placeholder='Maximum bonus amount' @input="validateBonusMaxAmount"/>
                                            <span class="text text-primary font-size-13">{{ __('set_0_if_you_want_to_remove_limit') }}</span>
                                            <span v-if="bonusMaxAmountValidationError" class="error">{{ bonusMaxAmountValidationError }}</span>
                                        </div>
                                    </div>
                                </div>
                            </div>
...
export default {
    ...
    methods: {
        ...
        validateBonusMaxAmount() {
            ...
            // Check if max amount is less than min amount (when both are set and > 0)
            ...
            // Check if max amount is negative
            if (maxAmount < 0) {
        ...
        changeBonusType(){
            if(this.deliveryBoys.bonus_type == 0){
                ...
                // Clear validation errors when switching to fixed salary
                this.bonusMinAmountValidationError = null;
                this.bonusMaxAmountValidationError = null;
            }else{
        ...
        saveRecord: function(){
            ...
            // Validate bonus amounts before saving
            if(this.deliveryBoys.bonus_type == 1) {
                this.validateBonusMinAmount();
                this.validateBonusMaxAmount();
                
                // Check if there are any validation errors
                if(this.bonusMinAmountValidationError || this.bonusMaxAmountValidationError) {
                    this.showError("Please fix the bonus amount validation errors before saving.");
                    return;
                }
            }
